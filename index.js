...
function createRock(x) {
  ...
  function moveRock() {
    ...
    // let top = positionToInteger(rock.style.top)
     let localTop = positionToInteger(rock.style.top) ;
     ...
     */
     ...
     else if (localTop < 400){
      // console.log(`2`)
      
      // console.log(`2 top=${localTop}`)
      rock.style.top = `${localTop += 2}px`;
      if (top > 0) {
...
function moveDodgerRight() {
  ...
  function moverRight() {
      DODGER.style.left = `${left += 4}px`
      if (left < 360) {
        window.requestAnimationFrame(moverRight)
    }
  }
  window.requestAnimationFrame(moverRight)
}
